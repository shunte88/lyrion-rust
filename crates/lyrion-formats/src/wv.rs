//! WavPack format parser using lofty
...
impl FormatParser for WavPackParser {
    fn parse(&self, path: &str) -> Result<AudioMetadata> {
        ...
        // Extract artwork from APEv2 tags (WavPack uses APEv2)
        if let Some(tag) = tag {
...
mod tests {
    ...
    #[test]
    fn test_wavpack_parser_supports() {
        let parser = WavPackParser;
        assert!(parser.supports_extension("wv"));
        assert!(parser.supports_extension("WV"));
