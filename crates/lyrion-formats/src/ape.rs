//! APE (Monkey's Audio) format parser using lofty
...
impl FormatParser for ApeParser {
    fn parse(&self, path: &str) -> Result<AudioMetadata> {
        ...
        // Extract artwork from APEv2 tags
        if let Some(tag) = tag {
...
mod tests {
    ...
    #[test]
    fn test_ape_parser_supports() {
        let parser = ApeParser;
        assert!(parser.supports_extension("ape"));
        assert!(parser.supports_extension("APE"));
        assert!(!parser.supports_extension("flac"));
    }
}
